...
export const AuthProvider = ({ children }) => {
  ...
  // Check if user is logged in on component mount (from localStorage and cookie)
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        // Check if token cookie exists by making a request to the server
    ...
    >
      ...
      <AlertDialog open={showAuthAlert} onOpenChange={setShowAuthAlert}>
        <AlertDialogContent className="bg-blue-50 border border-blue-100">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-blue-900">Login Required</AlertDialogTitle>
            <AlertDialogDescription className="text-blue-700">
              You need to login to access healthcare features. Please log in to continue.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction
