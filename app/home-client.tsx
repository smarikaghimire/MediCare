...
import BookingGuideModal from "@/components/booking-guide-modal"; // Import the modal component
...
const HomeClient = () => {
  ...
  const { requireAuth } = useAuth(); // Use the auth context
  ...
  // Handle find doctor click with auth check
  const handleFindDoctor = (e) => {
    e.preventDefault();
    if (requireAuth("/doctors")) {
      router.push("/doctors");
    }
  };

  // Modified: Handle appointment booking with auth check to show modal
  const handleBookAppointment = (e) => {
    e.preventDefault();
    if (requireAuth("/appointment")) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-50 to-white font-inter">
      ...
      <section
        className="container mx-auto px-6 py-8 mt-6 relative z-10"
      ...
      >
        <h2
          id="services-heading"
          className="text-4xl font-bold text-gray-800 mb-16 text-center font-outfit"
        >
          Our Healthcare{" "}
      ...
      >
        <h2
          id="cta-heading"
          className="text-4xl font-bold text-gray-800 mb-8 font-outfit"
        >
          Ready to Get{" "}
          <span className="bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">
            Started?
        ...
        <div className="flex justify-center gap-6">
          <button
            className="bg-blue-600 text-white px-8 py-4 rounded-xl font-semibold hover:bg-blue-700 transition-all duration-300 flex items-center transform hover:scale-105 shadow-lg hover:shadow-xl"
            onClick={(e) => {
              e.preventDefault();
              if (requireAuth("/doctors")) {
                router.push("/doctors");
